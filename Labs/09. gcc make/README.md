...
|--|--|--|
|e\_ident[0] | '\ x7f' | ``Магическое'' значение (константа)|
|e\_ident[1] | 'E' | ``Магическое'' значение (константа)|
|e\_ident[2] | 'L' | ``Магическое'' значение (константа)|
|e\_ident[3] | 'F' | ``Магическое'' значение (константа)|
|e\_ident[4] | 1 | Размер слова в битах: 0 - неизвестно, 1 - 32, 2 - 64|
|e\_ident[5] | 1 | Порядок байт: 0 - неизвестно, 1 - little-endian, 2 - big-endian|
|e\_ident[6] | 1 | Версия формата ELF: 0 - неизвестно, 1 - текущая версия|
|e\_ident[7] | 0 | ОС и бинарный интерфейс, для Linux - 0|
|e\_ident[8] | 0 | Версия бинарного интерфейса, для Linux - 0|
|e\_ident[9-15] | 0 | Зарезервировано|
...
## Сборка самостоятельной прошивки

Дан репозиторий `training_camp`. Данный репозиторий содержит код системы на кристалле с процессорным ядром и периферией, а также startup-файлы, скрипты для компоновщика и драйвера периферии. Необходимо дописать make-файл для сборки ПО проверяющего работу контроллера gpio в СнК, используя полученные знания. Итогом работы будет elf файл ПО, который в следующей лабораторной работе будет загружаться в память СнК по интерфейсу JTAG.

Внутри репозитория есть папка sw. В ней находится весь код, относящийся к разработке ПО под данную СнК. В папке headers находится файл `pulpino.h`, связывающий адресное пространство системы на кристалле с программным обеспечением. В папке `pulpino_testsw` находится прототип make-файла для сборки ПО. В той же директории находится файл `gpio_test_main.c` – исходный код программы, исполняемой после запуска устройства.

В папке `pulpino_testsw` находится папка `board_support/pulpino_hwlib`, в которой находятся драйверы для работы периферийных устройств В той же директории находится папка `startup`, хранящая весь необходимый startup-код. Для компиляции вам необходимы файлы `crt0.riscv.S` и `link.common.ld`.
Результатом выполнения задания является `*.elf` и `disasm.S` файлы вашего ПО.
