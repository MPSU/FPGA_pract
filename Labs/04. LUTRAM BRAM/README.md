...
Во многих задачах бывает полезно задать начальное содержимое памяти ещё до начала работы схемы. Это называется **инициализацией памяти**. После конфигурирования ПЛИС такие данные оказываются записанными в массив памяти, и модуль может сразу выполнять чтение заранее подготовленных значений.

Инициализация может использоваться как для LUTRAM, так и для BRAM, если память описана в виде массива и корректно синтезируется. Для задания начального содержимого памяти используется системная функция `$readmemh`, которая считывает значения из файла и записывает их в массив памяти.

У функции `$readmemh` есть два обязательных аргумента:

- имя файла с данными
- имя массива памяти, который нужно инициализировать

И существуют два необязательных аргумента:

- начальный адрес загрузки
- конечный адрес загрузки

Полная форма вызова выглядит так:

```verilog
$readmemh("<file_name>", <memory_name>, <start_addr>, <end_addr>);
```

На практике чаще всего используется сокращённый вариант:

```verilog
$readmemh("<file_name>", <memory_name>);
```

Ниже приведён пример однопортовой памяти с синхронной записью, синхронным чтением и инициализацией.
...
Файл инициализации представляет собой обычный текстовый файл. Каждая строка обычно соответствует одной ячейке памяти. Например, файл `mem_init.mem` может иметь такой вид:
...
Файл с данным примером можно найти [по ссылке](./examples/04_bram_dp_true/bram_dp_true_2clk.sv).
